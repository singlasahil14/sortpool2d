...
from sort_pool2d import sort_pool2d
...
class Classifier:
    def __init__(self, batch_size, layer_sizes, num_channels=1, pool_range=1):
        """
        ...
        with tf.variable_scope('g', reuse=self.reuse):
            ...
            with tf.variable_scope('conv_layers'):
                with tf.variable_scope('g_conv1'):
                    ...
                    g_conv1_encoder = sort_pool2d(g_conv1_encoder, k=self.pool_range, padding='SAME')
                ...
                with tf.variable_scope('g_conv2'):
                    ...
                    g_conv2_encoder = tf.contrib.layers.batch_norm(g_conv2_encoder, updates_collections=None,
                                                                   decay=0.99,
                                                                   scale=True, center=True, is_training=training)
                    g_conv2_encoder = sort_pool2d(g_conv2_encoder, k=self.pool_range, padding='SAME')
                ...
                with tf.variable_scope('g_conv3'):
                    ...
                    g_conv3_encoder = tf.contrib.layers.batch_norm(g_conv3_encoder, updates_collections=None,
                                                                   decay=0.99,
                                                                   scale=True, center=True, is_training=training)
                    g_conv3_encoder = sort_pool2d(g_conv3_encoder, k=self.pool_range, padding='SAME')
                ...
                with tf.variable_scope('g_conv4'):
                    ...
                    g_conv4_encoder = tf.contrib.layers.batch_norm(g_conv4_encoder, updates_collections=None,
                                                                   decay=0.99,
                                                                   scale=True, center=True, is_training=training)
                    g_conv4_encoder = sort_pool2d(g_conv4_encoder, k=self.pool_range, padding='SAME')
...
class MatchingNetwork:
    def __init__(self, support_set_images, support_set_labels, target_image, target_label, keep_prob,
                 ...
                 num_samples_per_class=1, pool_range=1):
        ...
        self.batch_size = batch_size
        self.fce = fce
        self.g = Classifier(self.batch_size, num_channels=num_channels, layer_sizes=[64, 64, 64 ,64], pool_range=pool_range)
        if fce:
