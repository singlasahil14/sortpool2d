...
class LeNet(object):
  ...
  def __init__(self, image_size, num_channels, hidden_dim, pool_range=1):
  ...
  def core_builder(self, x):
    ...
    pool1 = sort_pool2d(relu1, k=self.pool_range, padding='SAME', name='pool1')
    ...
    pool2 = sort_pool2d(relu2, k=self.pool_range, padding='SAME', name='pool2')
...
class Model(object):
  ...
  def __init__(self, input_dim, output_dim, rep_dim, memory_size, vocab_size,
               learning_rate=0.0001, use_lsh=False, pool_range=1):
  ...
  def get_embedder(self):
    return LeNet(int(self.input_dim ** 0.5), 1, self.rep_dim, self.pool_range)
